...
- FR-14: Pengguna dapat mengunggah/scan foto struk; sistem mengekstrak nominal & item lewat AI (Claude API, model Haiku) untuk mempercepat input. Hasil ekstraksi wajib direview/dikonfirmasi pengguna sebelum resmi tersimpan sebagai transaksi (lihat FR-14a).
...
- AI untuk scan struk memakai **Claude API dengan model Haiku** (`claude-haiku-4-5-20251001`) — dipilih karena biaya per-token termurah di lineup Claude dan cukup untuk task ekstraksi terstruktur (bukan reasoning kompleks). Dipanggil dari server (Route Handler/Server Action Next.js), bukan langsung dari client, agar API key tidak pernah terekspos ke browser.
- Biaya berjalan berbasis pemakaian (pay-as-you-go per token); tidak ada tier gratis permanen untuk produksi. Akun API perlu diisi saldo (billing) sebelum dapat memproses request sungguhan — kredit awal $0 hanya untuk evaluasi.
- Estimasi biaya per scan struk dengan Haiku: di bawah $0.01 (gambar terkompresi + prompt terstruktur ringkas), sehingga tetap murah untuk skala MVP/early-stage.
